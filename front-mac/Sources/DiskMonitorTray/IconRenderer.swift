...
// Mirror of crates/disk-monitor-tray/src/icon/render.rs for the donut geometry,
...
//    clock, battery, wifi, etc.
// 2. The donut colors stay saturated and convey state on their own — that's
//    where the "disk almost full" signal lives, not in the text.
...
/// Picks the donut "used" color from disk utilization, matching `render.rs::used_color`.
...
/// Short, panel-friendly version of a mount point. `/` → `/`, `/home` → `home`,
/// `/media/user/seagate2T` → `seagat`. Truncated to 6 chars to keep the icon
/// from dominating the menu bar.
...
/// Fractional sizes produce blurry glyphs.
...
struct IconRenderer {
    ...
    /// Per-mount block: `[icon] 2 [label] 2 [donut]`. Each block is sized to
    /// its own label width — mount points are static within a session, so
    /// there's no jitter from frame to frame.
    ...
    private func layout(mounts: [Mount], scale: CGFloat, connected: Bool, appearance: IconAppearance, compact: Bool) -> Layout {
        ...
        let blocks: [Block] = mounts.map { m in
            ...
            // In compact mode, the per-block label is omitted so its measured
            // width is zero — donut sits flush against the icon.
        ...
        let total: CGFloat
        if blocks.isEmpty {
            // Disconnected / connecting: icon + dash, no donut.
    ...
    private func draw(layout: Layout, ctx: CGContext, connected: Bool) {
        if layout.blocks.isEmpty {
            // Connecting / disconnected: icon (dimmed) + dash. No donut — a grey
            // ring at "0%" reads like a real reading when there is none.
            var x: CGFloat = 0
    ...
    private static func font(size: CGFloat) -> CTFont {
        // monospacedDigit (not monospaced): SF Pro letters with fixed-width
        // digits, matching the system menu bar (clock, battery, …).
