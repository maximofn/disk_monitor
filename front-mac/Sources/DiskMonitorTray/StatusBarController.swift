...
private let repoURL = URL(string: "https://github.com/maximofn/disk_monitor")!
private let coffeeURL = URL(string: "https://www.buymeacoffee.com/maximofn")!
private let compactModeDefaultsKey = "DiskMonitorTray.compactMode"
...
final class StatusBarController: NSObject {
    ...
    private let logger = Logger(subsystem: "com.maximofn.disk-monitor", category: "tray")
    ...
    init(renderer: IconRenderer, backendURL: String) {
        ...
        if let button = statusItem.button {
            ...
            button.toolTip = "Disk Monitor — connecting to \(backendURL)"
        }
        // Subscribe to system-wide light/dark toggle. Do NOT KVO
        // `effectiveAppearance` on the button: AppKit re-evaluates that during
        // repaints, making a tight feedback loop with refreshIcon.
    ...
    private func renderKey(mounts: [Mount], connected: Bool, appearance: IconAppearance) -> String {
        ...
        for m in mounts {
            // Visible inputs only: mount path + used percent rounded.
    ...
    private func mountSubmenu(for mount: Mount) -> NSMenu {
        ...
        if mount.largestFiles.isEmpty {
            if let scanned = mount.largestFilesScannedAt {
                ...
                m.addItem(disabledItem("Largest files: scanning…"))
            }
        } else {
    ...
    @objc private func rescanMount(_ sender: NSMenuItem) {
        ...
        let log = logger
        // Detached so we don't block the menu close. The daemon coalesces
        // bursts of rescan POSTs, so a duplicate click is a no-op.
        Task.detached {
