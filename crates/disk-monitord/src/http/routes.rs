...
use axum::Json;
use disk_monitor_core::{Mount, Snapshot};
use serde::Serialize;
...
pub async fn info(State(state): State<AppState>) -> Json<InfoResponse> {
    ...
    Json(InfoResponse {
        ...
        api_version: disk_monitor_core::API_VERSION,
...
/// stripped by axum (e.g. `/v1/mounts/home` matches `/home`).
pub async fn mount(
...
pub async fn rescan_one(
    ...
    let tx = state.rescan_tx.as_ref().ok_or(StatusCode::SERVICE_UNAVAILABLE)?;
    let needle = format!("/{}", path.trim_start_matches('/'));
    // Only accept paths that match a known mount point so we don't queue
    // walks of arbitrary directories.
    let known = state
        .snapshot_rx
        .borrow()
        .mounts
