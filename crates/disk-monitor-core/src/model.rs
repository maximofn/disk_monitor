...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub struct Mount {
    /// Mount point as listed in `/proc/mounts` (e.g. `/`, `/home`).
    pub mount_point: String,
    /// Block device backing this mount (e.g. `/dev/nvme0n1p2`).
    pub device: String,
    ...
    /// Files we cannot stat (EACCES, ENOENT during the walk) are skipped.
    #[serde(default)]
...
mod tests {
    ...
    fn snapshot_roundtrips_through_json() {
        let snap = Snapshot {
            timestamp: "2026-05-03T11:00:00Z".to_string(),
            host: "carbon".into(),
            mounts: vec![Mount {
                mount_point: "/".into(),
                ...
                largest_files: vec![FileEntry {
                    path: "/var/log/journal/system.journal".into(),
                    size_bytes: 1_073_741_824,
                }],
                largest_files_scanned_at: Some("2026-05-03T11:30:00Z".into()),
            }],
        };
    ...
    fn old_snapshot_without_largest_files_still_decodes() {
        // Backwards compat: producers that pre-date the largest-files field
        // should still deserialize cleanly thanks to #[serde(default)].
        let json = r#"{
