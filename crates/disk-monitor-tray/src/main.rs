...
use tray::{DiskTray, State};

const ASSET_ICON_REL: &str = "assets/disk.png";
...
async fn main() -> Result<()> {
    ...
    let icon_path = locate_icon().context("could not find disk.png; install via `assets/`")?;
    ...
    let tray_state = DiskTray::new(renderer, cfg.backend_url.clone(), cache_dir)?;
...
async fn dump_icon_once(
    ...
    let snapshot: disk_monitor_core::Snapshot = reqwest::get(&url)
        .await
    ...
    let rendered = renderer.render(&snapshot.mounts, true);
...
fn cache_icon_dir() -> Result<PathBuf> {
    ...
    Ok(base.join("disk-monitor").join("icons"))
}

fn locate_icon() -> Option<PathBuf> {
    let mut candidates: Vec<PathBuf> = Vec::new();
    if let Some(path) = std::env::var_os("DISK_MONITOR_TRAY_ICON") {
    ...
    if let Some(data) = xdg_data {
        candidates.push(data.join("disk-monitor").join("disk.png"));
    }
    candidates.push(PathBuf::from("/usr/share/disk-monitor/disk.png"));
