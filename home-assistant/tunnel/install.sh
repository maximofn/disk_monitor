#!/usr/bin/env bash
# Instala el túnel SSH forward (raspihome → wallabot, puerto 9126) como user
# systemd unit. Diseñado para correr EN raspihome.

set -euo pipefail

UNIT_NAME="disk-monitor-ha-tunnel.service"
UNIT_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/systemd/user"
SRC_UNIT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/$UNIT_NAME"
TUNNEL_KEY="$HOME/.ssh/id_ed25519_disk_tunnel"
...
case "${1:-install}" in
    install)
        ...
        if [ ! -f "$TUNNEL_KEY" ]; then
            ...
            ssh-keygen -t ed25519 -N "" -f "$TUNNEL_KEY" -C "raspihome-disk-tunnel" >/dev/null
            echo
